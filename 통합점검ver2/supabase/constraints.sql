-- schema.sql 실행 후 적용합니다.
-- 이 파일은 이미 schema.sql만 실행된 원격 프로젝트에서도 안전하게 이어서 실행할 수 있습니다.
-- DROP, DELETE, TRUNCATE는 포함하지 않습니다.

-- company_aliases_natural_key는 UNIQUE 제약의 backing index 이름이기도 합니다.
-- 같은 이름의 기존 고유 인덱스가 정확히 같은 열을 가리킬 때만 제약에 연결합니다.
do $$
declare
    relation_kind "char";
begin
...
-- schema.sql을 먼저 실행한 기존 원격 프로젝트용 기본 foreign key 보완입니다.
-- 새 프로젝트에서는 schema.sql에 같은 이름으로 선언되어 있으므로 아무 작업도 하지 않습니다.
do $$ begin
...
-- report_items_order_unique도 backing index 이름 충돌을 안전하게 처리합니다.
do $$
declare
    relation_kind "char";
begin
